...
app.use(
  cors({
    origin: [
      "http://localhost:3000",
      "chrome-extension://mggcnpciocmgadnfpkooinmkgikobmoi",
    ],
...
const {
  ...
  check_login,
  editdetails,
} = require("./Controllers/authenticatetoken");
...
app.post("/signupform", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: "Error while saving data", details: error.message });
  }
});
...
app.post("/login", async (req, res) => {
  try {
    ...
    if (!user) {
      return res
        .status(404)
        .json({ error: "You are not registered, kindly register." });
    }
...
app.post("/generate-email", authenticateToken, async (req, res) => {
  ...
  if (!jobDescription || !applicantData) {
    return res
      ...
      .json({
        error:
          "Both jobDescription and applicantData are required in the request body.",
      });
  }
  try {
    const generatedEmail = await generateApplicationEmail(
      jobDescription,
      applicantData
    );
